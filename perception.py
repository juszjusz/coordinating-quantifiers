...
class Category:
    ...
    def __init__(self, id, rxr, ri, nk_list):
        self.id = id
        self.weights = []
        self.reactive_indicies = []
        self.rxr = rxr
        self.ri = ri
        self.nk_list = map(lambda nk: Fraction(nk[0], nk[1]), nk_list)
        self.x_left = float("inf")
        self.x_right = float("-inf")

    def response(self, stimulus):
        # index = self.nk_list[stimulus]
        # np.where(self.nk_list == stimulus)
        index = self.nk_list.index(stimulus.nk)
        return sum([w * self.rxr[ru_index][index] for ru_index, w in zip(self.reactive_indicies, self.weights)])

    def add_reactive_unit(self, stimulus, weight=0.5):
        self.weights.append(weight)
        self.reactive_indicies.append(self.nk_list.index(stimulus.nk))
    ...
    def reinforce(self, stimulus, beta):
        index = self.nk_list.index(stimulus.nk)
        self.weights = [w + beta * self.rxr[ru_index][index] for ru_index, w in zip(self.reactive_indicies, self.weights)]
