...
class AssociativeMatrix:
    ...
    def set_value(self, row, col, value):
        ...
        if max(self.matrix.flat) > 1.0:
            self.matrix = divide(self.matrix, max(self.matrix.flat))

    def forget(self, forgetting_factor, super_alpha, word_index):
        self.matrix = self.matrix - self.matrix * forgetting_factor

        to_forget = [j for j in range(self.matrix.shape[0])
                     if max(self.matrix[j]) < super_alpha and j != word_index]

        if len(to_forget):
            self.matrix = delete(self.matrix, to_forget, axis=0)
